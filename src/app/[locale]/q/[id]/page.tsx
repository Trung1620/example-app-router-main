...
export default async function PublicQuote({params}: PublicQuotePageProps) {
  const {locale, id} = await params;

  const q = await prismadb.quote.findUnique({
    where: {id},
    include: {items: true, customer: true}
  });
  if (!q) return <div className="p-6">Không tìm thấy báo giá.</div>;

  // ✅ chỉ READ delivery (public)
  ...
  return (
    <div className="p-4 md:p-6 print:p-0 flex flex-col items-center">
      ...
      {/* ✅ Public actions: CHỈ in */}
      <div className="no-print w-full max-w-4xl flex justify-end mb-2 gap-2">
        <PrintButton />
      </div>

      <div className="sheet w-full max-w-4xl bg-white p-4 md:p-6 shadow">
        {/* Header */}
        {/* Header (compact) */}
          ...
          <div className="col-span-8 md:col-span-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="border rounded px-2 py-1 leading-tight print:px-1 print:py-[2px]">
                ...
                <div className="text-sm font-semibold print:text-[11px]">
                  {new Date((q as any).createdAt).toLocaleDateString(locale)}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ===================== ITEMS ===================== */}
        {/* (Giữ nguyên phần ITEMS / Totals / Sign y hệt code của bạn) */}
          ...
          <div>
            ...
            {(q as any).validUntil && (
              <div className="mt-3 italic">
                ...
                <div>
                  {new Date((q as any).validUntil).toLocaleDateString(locale)}
                </div>
              </div>
            )}
          </div>
...
function BilingualLabel({en, vi}: {en: string; vi: string}) {
  return (
    <div className="leading-tight">
      <div className="text-xs font-medium text-gray-600">{en}</div>
