...
it('sets caching headers', async ({request}) => {
  for (const pathname of ['/en', '/en/pathnames', '/vi', '/vi/pfadnamen']) {
...
it('sets a cookie when necessary', async ({page}) => {
  ...
  await page
    ...
    .selectOption({value: 'vi'});
  await expect(page).toHaveURL('/vi');
  expect(await getCookieValue()).toBe('NEXT_LOCALE=vi');
});
...
it("sets a cookie when requesting a locale that doesn't match the `accept-language` header", async ({
  ...
  expect(value).toContain('NEXT_LOCALE=vi;');
...
<xhtml:link rel="alternate" hreflang="vi" href="http://localhost:3000/vi" />
</url>
<url>
...
<xhtml:link rel="alternate" hreflang="vi" href="http://localhost:3000/de/pfadnamen" />
</url>
<url>
